...
object Utilities {
  ...
  def countMatching[T, U] (seq: Seq[T]) (f: PartialFunction[T, U]) = seq.collect (f).size
  def percentMatching[T, U] (seq: Seq[T]) (f: PartialFunction[T, U]) = countMatching (seq)(f).toDouble / seq.size
  def percentEqual[T] (seq: Seq[T], value: T) = seq.count (_ == value).toDouble / seq.size
}
